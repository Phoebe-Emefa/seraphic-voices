...
import { FaClock, FaMapMarkerAlt, FaRegCalendarAlt, FaVideo } from "react-icons/fa";
...
const EventDetailsModal = ({ isOpen, onClose, event }: EventDetailsModalProps) => {
  ...
  const isMobileLayout = useBreakpointValue({ base: true, lg: false }) ?? true;
  const [origin, setOrigin] = useState("");

  const image = event ? resolveEventImage(event) : "";
  const ticket = httpsUrl(event?.ticket_url);
  const video = httpsUrl(event?.video);
  const videoId = useMemo(() => extractYouTubeId(video || ""), [video]);
  const embedUrl = useMemo(() => {
    if (!videoId || !origin) return null;
    return buildYouTubeEmbedUrl(videoId, origin, { autoplay: false });
  }, [videoId, origin]);

  const upcoming = event ? isUpcomingEvent(event) : false;
  ...
  const showTicketCta = Boolean(ticket && upcoming);
  ...
  const poster = (
    ...
    >
      ...
      <IconButton
        ...
        _hover={{ bg: "rgba(4, 25, 68, 0.9)", color: "white" }}
        onClick={onClose}
      />
      {badge ? (
        <VStack
  ...
  const details = (
    <VStack align="stretch" spacing={{ base: 5, md: 6 }}>
          ...
          <VStack align="stretch" spacing={4} position="relative" zIndex={1}>
            <VStack align="flex-start" spacing={1}>
              ...
              <Text
                ...
                lineHeight={1.25}
              >
                Reserve your seat for this performance
              </Text>
              <Text fontSize="sm" color="whiteAlpha.800">
                {formatEventDate(event.start_date)}
                {badge ? ` · ${badge.format("dddd")}` : ""}
              </Text>
            </VStack>
            <CustomButton
      ...
      {embedUrl ? (
        <Box>
          <HStack spacing={2} mb={3} color="secondary.500">
            <Icon as={FaVideo} boxSize={3.5} />
            <Text
              fontSize="2xs"
              fontWeight="bold"
              letterSpacing="0.16em"
  ...
  return (
    ...
    >
      <ModalOverlay bg="rgba(3, 15, 38, 0.92)" backdropFilter="blur(16px)" />
      <ModalContent
        bg="#041a42"
        color="white"
        ...
        maxH={{ base: "100dvh", lg: "94vh" }}
        maxW={{ base: "100vw", lg: "min(96vw, 76rem)" }}
        display="flex"
      ...
      >
        ...
        >
          {isMobileLayout ? (
            ...
            <Grid templateColumns="1fr 1.1fr" flex="1" minH="28rem" alignItems="stretch">

...
const UpcomingEventCard = ({
  ...
  const statusLabel = tone === "past" ? "Past performance" : "Upcoming concert";
  const isPast = tone === "past";
  const ticket = httpsUrl(event.ticket_url);
  const canPurchase =
    showTicketCta && Boolean(ticket) && isUpcomingEvent(event) && !isPast;

  const handleOpenDetails = () => onOpen();

  const modal = (
    <EventDetailsModal isOpen={isOpen} onClose={onClose} event={event} />
  );
